...
namespace GTAExpansion
{
    ...
    Vest.btnColor2
        ...
        label_59:
            ...
            if (player.Armor > 0)
            {
                if (!Vest.mainMenu.MenuItems.Contains((UIMenuItem)RemoveArmor) && HasPedBoughtVest(player))
                    Vest.mainMenu.AddItem((UIMenuItem)RemoveArmor);
                
                if (!Vest.mainMenu.MenuItems.Contains((UIMenuItem)ReplaceArmorPlate) && player.Armor < 100 && HasPedBoughtVest(player) && Vest.getArmorPlateCount(player, "Armor_Plates") > 0)
                ...
                {
                     if (!Vest.mainMenu.MenuItems.Contains((UIMenuItem)EquipArmor) && HasPedBoughtVest(player) && armortakenoff)
            ...
            Vest.mainMenu.OnItemSelect += (ItemSelectEvent)((sender, item, index) =>
                {
                    ...
                    if (item == EquipArmor)
                    {
                        if (!ped.IsSittingInVehicle() || ped.IsSittingInVehicle() && !ped.CurrentVehicle.IsBicycle && !ped.CurrentVehicle.IsBike && !ped.CurrentVehicle.IsQuadBike)
                        {
                            Vest.inMenu = false;
                            player.Task.ClearAll();
                            player.Armor = GetArmorLevelFromXml();
                            showVestModel();
                            armortakenoff = false;
                            Vest.mainMenu.RefreshIndex();
                            Vest.modMenuPool.CloseAllMenus();
                    ...
                    if (item == ReplaceArmorPlate)
                    {
                        ...
                        Vest.SavePlates(player, "Armor_Plates", Vest.armorPlateCount);
                        Vest.inMenu = false;
                        player.Armor = 100;
                        Vest.modMenuPool.CloseAllMenus();
                    ...
                    Vest.inMenu = false;
                    //Vest.weaponInventoryAnim(Vest.bagModelReturn(player), player);
                });
        }
        ...
        public static void showVestModel()
        {
            Ped Player = Game.Player.Character;

            int playerId = Helpers.GetPlayerID();
            int component = Helpers.CharacterClothes[playerId, 0];
            int drawable = Game.Player.Character.Armor > 0
                ? Helpers.CharacterClothes[playerId, 1]
                : Helpers.CharacterClothes[playerId, 2];

            Function.Call(Hash.SET_PED_COMPONENT_VARIATION, new InputArgument[4]
            {
        (InputArgument)(Entity)Player,
        (InputArgument)component,
        (InputArgument)drawable,
        (InputArgument)0
            });
        ...
        public static void vestcheck()
        {
            ...
            if (armor > 0 && !HasPedBoughtVest(player))
            {
                Common.UpdateAttachment("vest", true);
                
            }
            if (HasPedBoughtVest(player))
            {
        ...
        public static void vestcheck()
        {
           ...
           //     return;
            if (armor > 0 && (pedElement.Attribute("vest") == null || (armor > 0 && pedElement.Attribute("vest").Value != "true")))
            {
