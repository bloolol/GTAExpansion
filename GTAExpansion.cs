        ...
        private void OnTick(object sender, EventArgs e)
        {
            ...
            if (HungerSystem.hungerModuleActive && HungerSystem.hungerTimeRef < Game.GameTime)
            {
                ...
                if (HungerSystem.curHungerLvl <= HungerSystem.criticalHungerLvl)
                {
                    ...
                    {
                        HTools.Main.Notify("You're ~r~starving~w~.~n~Eat some food using the ~b~Main ~y~Menu~w~", "Health Report", 0, (int)byte.MaxValue, 0, NotificationIcon.LesterDeathwish);
            ...
            if (HungerSystem.hungerModuleActive && HungerSystem.thirstTimeRef < Game.GameTime)
            {
                ...
                if (HungerSystem.curThirstLvl <= HungerSystem.criticalThirstLvl)
                {
                    ...
                    {
                        HTools.Main.Notify("You're ~r~starving~w~.~n~Drink fluids using the ~b~Main ~y~Menu~w~", "Health Report", 0, (int)byte.MaxValue, 0, NotificationIcon.LesterDeathwish);
                        HTools.Main.startBleeding(Game.Player.Character, false, useInjuryAnim: false);
                    }
            ...
            if (Common.findSellerOption)
            {
                ...
                int num26 = 176;
                int num27 = 177;
                Screen.ShowHelpText("~y~Call ~w~closest ~BLIP_INFO_ICON~ Dealer to purchase ~r~supplies~w~, ~r~bags~w~ and ~r~holsters", 5000);
