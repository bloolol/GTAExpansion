    ...
    public class GTAExpansion : Script
    {
        ...
        private void OnTick(object sender, EventArgs e)
        {
            ...
            if (!Common.loaded)
            {
                ...
                {
                    Common.callContact = new HPhoneContact("Inventory menu");
                ...
                if (FocusMode.targetsPicked)
                {
                    ...
                    if (FocusMode.focusEffectsStoped)
                    {
                        if (FocusMode.focusTargetedPeds.Length != 0 && Game.Player.Character.Weapons.Current.Ammo >= FocusMode.focusTargetedPeds.Length && FocusMode.focusTargetingTimer > 0)
                        {
                            ...
                            {
                                if (FocusMode.targetCam == (Camera)null && (GameplayCamera.IsRendering || World.RenderingCamera == ShoulderCameraSwitch.ShoulderCamera))
                                {
            ...
            if (WoundsSystem.woundsModuleActive && WoundsSystem.woundsTimer < Game.GameTime)
            {
                ...
                if (!WoundsSystem.isWounded && Game.Player.Character.HasBeenDamagedByAnyWeapon())
                {
                    if (Common.rnd.Next(0, 5) == 0)
                    {
                        ...
                        Screen.ShowHelpText("~BLIIP_INFO_ICON~ You have been ~r~heavily wounded~w~.~n~ Use some painkillers to stop pain!", 5000);
                    }
                    else
                        Function.Call(Hash.CLEAR_PED_LAST_WEAPON_DAMAGE, (InputArgument)(Entity)Game.Player.Character);
                }
                if (WoundsSystem.isWounded)
                {
                    if (WoundsSystem.bleedTimer <= 0 || (Entity)Game.Player.Character != (Entity)WoundsSystem.curWoundPlayer || Game.Player.Character.IsDead)
                    {
                        ...
                        Screen.ShowHelpText("~BLIIP_INFO_ICON~ The ~r~bleeding~w~ stoped", 5000);
                    }
                    else
                    {
                ...
                if (!Function.Call<bool>(Hash.IS_ENTITY_PLAYING_ANIM, (InputArgument)(Entity)Game.Player.Character, (InputArgument)"move_action@p_m_one@armed@2h_short@trans@a", (InputArgument)"idle2action", (InputArgument)3))
                {
                    if (!Game.Player.Character.IsInCover && Game.Player.Character.IsOnFoot && Game.Player.CanControlCharacter)
                    {
                        ...
                        if (flag1)
                        {
                            ...
                            Screen.ShowHelpText("~BLIP_INFO_ICON~ ~r~The silencer cant be attached to this weapon", 5000);
                    }
                    else
            ...
            if (Game.Player.Character.IsOnFoot && Game.Player.Character.IsAiming && !FocusMode.isFocused && Game.Player.CanControlCharacter && !Silencer.toggleSilencer)
            {
                ...
                if (Function.Call<bool>(Hash.IS_DISABLED_CONTROL_JUST_PRESSED, (InputArgument)0, (InputArgument)Silencer.silencer_toggle_btn))
                {
                ...
                if (ShoulderCameraSwitch.ShoulderCameraModuleActive)
                {
                    if (ShoulderCameraSwitch.ShoulderCameraActive)
                    {
                        ...
                        {
                            if (ShoulderCameraSwitch.ShoulderCamera == (Camera)null)
                            {
                                ShoulderCameraSwitch.ShoulderCamera = ShoulderCameraSwitch.switchShoulderCamera(Game.Player.Character, ShoulderCameraSwitch.ShoulderCamera);
                            }
                            else
                            {
                                ShoulderCameraSwitch.ShoulderCamera.Direction = GameplayCamera.Direction;
                                ShoulderCameraSwitch.ShoulderCamera.Rotation = GameplayCamera.Rotation;
                                ShoulderCameraSwitch.ShoulderCamera.FieldOfView = GameplayCamera.FieldOfView;
                            }
                            Function.Call(Hash.DISABLE_GAMEPLAY_CAM_ALTITUDE_FOV_SCALING_THIS_UPDATE);
                            Function.Call(Hash.SHOW_HUD_COMPONENT_THIS_FRAME, (InputArgument)14);
                        }
                        else
                        {
                            ...
                            if (World.RenderingCamera == ShoulderCameraSwitch.ShoulderCamera)
                                World.RenderingCamera = (Camera)null;
                            if (ShoulderCameraSwitch.ShoulderCamera != (Camera)null)
                            {
                                ShoulderCameraSwitch.ShoulderCamera.Delete();
                                ShoulderCameraSwitch.ShoulderCamera = (Camera)null;
                            }
                        }
                    }
                ...
                if (LaserSight.laserSightModeActive)
                {
                    ...
                    if (Function.Call<bool>(Hash.IS_DISABLED_CONTROL_JUST_PRESSED, (InputArgument)0, (InputArgument)LaserSight.laser_sight_toggle_btn))
                    {
                        ...
                        if (flag)
                        {
                            ...
                            Screen.ShowHelpText("~BLIP_INFO_ICON~ You need to set ~y~flashlight~w~ component to use laser sign with this weapon", 6000);
                    }
                }
                if (ShoulderCameraSwitch.ShoulderCameraModuleActive && Function.Call<bool>(Hash.IS_DISABLED_CONTROL_JUST_PRESSED, (InputArgument)0, (InputArgument)ShoulderCameraSwitch.switch_shoulder_camera_btn) && !Game.Player.Character.IsInCover)
                {
                    if (!ShoulderCameraSwitch.ShoulderCameraActive)
                    {
                        ShoulderCameraSwitch.ShoulderCamera = ShoulderCameraSwitch.switchShoulderCamera(Game.Player.Character, ShoulderCameraSwitch.ShoulderCamera);
                    ...
                    {
                        if (ShoulderCameraSwitch.ShoulderCamera != (Camera)null && World.RenderingCamera == ShoulderCameraSwitch.ShoulderCamera)
                        {
                            World.RenderingCamera = (Camera)null;
                            ShoulderCameraSwitch.ShoulderCamera.Delete();
                            ShoulderCameraSwitch.ShoulderCamera = (Camera)null;
            ...
            {
                LaserSight.laserSightActivationTimer = 20;
                if (ShoulderCameraSwitch.ShoulderCameraModuleActive && ShoulderCameraSwitch.ShoulderCamera != (Camera)null && World.RenderingCamera == ShoulderCameraSwitch.ShoulderCamera)
                {
                    World.RenderingCamera = (Camera)null;
                    ShoulderCameraSwitch.ShoulderCamera.Delete();
                    ShoulderCameraSwitch.ShoulderCamera = (Camera)null;
                    ...
                    if (Game.Player.Character.IsAiming && !Game.Player.Character.IsSittingInVehicle() && !FocusMode.targetsPicked && !FocusMode.isFocused && Game.Player.CanControlCharacter && !Game.Player.Character.IsAttached())
                    {
                        ...
                        if (Function.Call<bool>(Hash.IS_DISABLED_CONTROL_JUST_PRESSED, (InputArgument)0, (InputArgument)WeaponJamming.clean_weapon_btn) && WeaponJamming.cleaningRequired)
                        {
                            if (Game.Player.CanControlCharacter && Game.Player.Character.IsOnFoot && !Game.Player.Character.IsInCombat && Game.Player.WantedLevel <= 0)
                            {
                                ...
                                Screen.ShowHelpText("~BLIP_INFO_ICON~ You cant clean your weapon now!", 3000);
                        }
                    ...
                    if (WeaponJamming.isCheckingWeaponCondition)
                    {
                        if (Function.Call<bool>(Hash.IS_ENTITY_PLAYING_ANIM, (InputArgument)(Entity)Game.Player.Character, (InputArgument)str1, (InputArgument)animName1, (InputArgument)3))
                        {
                            ...
                            if ((double)Function.Call<float>(Hash.GET_ENTITY_ANIM_CURRENT_TIME, (InputArgument)(Entity)Game.Player.Character, (InputArgument)str1, (InputArgument)animName1) >= (double)num21)
                            {
                                ...
                                {
                                    str2 = "~n~You need to purchase a weapon cleaning tools from  ~p~weapon dealer~w~ to maintain your guns in good condition. Dial phone contact named ~p~Inventory menu~w~ to call a blackmarket Common.seller";
                                }
                                else
                                {
                                    index3 = Common.getSupplies(Game.Player.Character, "weapon_tools");
                                    str2 = "~n~You have ~o~" + index3.ToString() + "~w~ weapon cleaning tool.You can clean your weapon now.";
                                }
                                string str3 = str2;
            ...
            {
                Common.callContact.Name = "Inventory menu";
